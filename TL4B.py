...
class Config:
    ...
    TRANSFER_DATA_PATH = os.path.join(BASE_DIR, "B.csv")
...
if __name__ == "__main__":
    ...
    FREEZE_LAYERS_COUNT = 2 # 默认冻结所有隐藏层，只微调输出层

    OUTPUT_FOLDER_NAME = f"TL4B"
    ...
    if X_data_raw.shape[1] != Config.INPUT_FEATURES_COUNT:
        raise ValueError(f"B.csv数据特征数量 ({X_data_raw.shape[1]}) 与预期特征数量 ({Config.INPUT_FEATURES_COUNT}) 不匹配。")
